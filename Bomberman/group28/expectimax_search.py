...
class TestCharacter(CharacterEntity):
    ...
    def max_value(self, brd, val, level):
        if depth >= self.search_level:
    ...
    def es_val(self, brd, val, level):
        if level >= self.search_level:
            return self.get_score(brd, val)
        value = 0
        for loc in self.look_for_cell(s):
    ...
    def look_for_cell(self, brd):
        cells = []
        if self.wall_at(self.x + 1, self.y, brd):cells.append((brd, (self.x + 1, self.y)))
        if self.wall_at(self.x - 1, self.y, brd):cells.append((brd, (self.x - 1, self.y)))
        if self.wall_at(self.x, self.y + 1, brd):cells.append((brd, (self.x, self.y + 1)))
        if self.wall_at(self.x, self.y - 1, brd):cells.append((brd, (self.x, self.y - 1)))
        if self.wall_at(self.x + 1, self.y + 1, brd):cells.append((brd, (self.x + 1, self.y + 1)))
        if self.wall_at(self.x + 1, self.y - 1, brd):cells.append((brd, (self.x + 1, self.y - 1)))
        if self.wall_at(self.x - 1, self.y + 1, brd):cells.append((brd, (self.x - 1, self.y + 1)))
        if self.wall_at(self.x - 1, self.y - 1, brd):cells.append((brd, (self.x - 1, self.y - 1)))
